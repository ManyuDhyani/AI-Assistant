    ...
    while True:
        ...
        try:
            if question == "send message":
                voice_type.voice_typing()
            elif "recommend movie like" in question:
                x = question.replace("recommend movie like ","")
                content_re.content(x.title())
            elif "recommend similar movie" in question:
                x = question.replace("recommend similar movie ","")
                y = x.replace(x[-1],"")
                collaborate_re.collaborate(y.title(),int(x[-1]))
            elif "open" or "wikipedia" or "play" or "search" in question:
                navigation.process_text(question.lower())
                time.sleep(1)
            else:
                pass
#############################################
